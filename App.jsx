...
const NebulaCanvas = () => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      // Red atómica: grid plano cubriendo TODO el recuadro, nodos muy pequeños
      const spacing = 14; // px entre nodos
      ...
      for (let j = 0; j < rows; j++) {
        for (let i = 0; i < cols; i++) {
          grid[j * cols + i] = {
            ...
            tint: Math.random() < 0.22 ? "p" : "v",
          };
        }
      }
      // Bokeh minimal para profundidad
      const bokehCount = Math.min(8, Math.max(3, Math.floor((w * h) / 45000)));
    ...
    const draw = (t) => {
      ...
      // Atomic dots con wave + pulso de alpha (todo el canvas)
      for (const p of grid) {
        const wave =
          Math.sin(p.baseX * 0.011 + t * 0.0008) * 4 +
          Math.sin(p.baseX * 0.018 + p.baseY * 0.013 + t * 0.0005) * 3 +
          Math.sin(p.baseY * 0.016 + t * 0.0006) * 2.5;
        const y = p.baseY + wave;
        const pulse = 0.45 + 0.55 * Math.sin(t * 0.0012 + p.phase);
        const alpha = 0.08 + pulse * 0.2;
        ctx.fillStyle = p.tint === "p"
          ? `rgba(242,57,255,${alpha})`
          : `rgba(137,67,227,${alpha})`;
        ctx.fillRect(p.baseX, y, 0.8, 0.8);
      }

      // Bokeh muy sutil para profundidad
      ...
      <div className="reveal relative gradient-border-strong rounded-[2rem] p-8 md:p-14 overflow-hidden text-center">
        ...
        <div className="relative">
          <h2 className="font-bold text-[clamp(1.8rem,4.5vw,2.8rem)] leading-[1.1] tracking-[-0.02em]">
            ¿Listo para construir <span className="text-brand-gradient">tu agencia de IA</span>?
          </h2>
