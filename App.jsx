...
/**
 * Masterclass Nexum AI — Landing + Post-Registro
 * ------------------------------------------------------
...
const LiveBadge = () => (
  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full gradient-border text-[11px] tracking-[0.2em] uppercase font-semibold">
    ...
    <span className="text-white/90">Masterclass en vivo</span>
  </div>
);

const Nav = ({ countdown, onOpenModal }) => (
    ...
    <div className="max-w-[1180px] mx-auto grid lg:grid-cols-[1.2fr_1fr] gap-10 lg:gap-14 items-center">
      <div className="text-center lg:text-left" style={{ animation: "fadeUp .9s cubic-bezier(.2,.7,.2,1) both" }}>
        ...
        <blockquote className="mt-5 pl-4 border-l-2 border-[#8943E3]/60 text-white/55 text-[15px] italic max-w-xl mx-auto lg:mx-0 leading-relaxed">
          Todas las empresas necesitan IA. Casi nadie sabe cómo convertir eso en un negocio.
        </blockquote>
        ...
        <div className="mt-8 flex flex-col items-center lg:items-start gap-3">
          <button type="button" onClick={onOpenModal} className="cta-primary rounded-full px-7 py-4 font-semibold text-white text-sm sm:text-base tracking-wide inline-flex items-center gap-2">
...
const PainBlock = () => {
  const pains = [
    ...
    "No tienes una oferta clara que las empresas quieran comprar",
        ...
        <div ref={cardRef} onMouseMove={onMove} onMouseLeave={onLeave}
          ...
          <div className="relative text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full gradient-border text-[11px] tracking-[0.25em] uppercase font-semibold text-white/70">
              <span className="w-1.5 h-1.5 rounded-full bg-[#F239FF]" />
              El diagnóstico
            </div>
            <h2 className="mt-5 font-bold text-[clamp(1.8rem,4.8vw,3rem)] leading-[1.1] tracking-[-0.02em] max-w-[22ch] mx-auto">
              El problema no es la IA. Es que <span className="text-brand-gradient">no sabes monetizarla.</span>
            </h2>
          </div>

          <div className="relative mt-10 grid md:grid-cols-2 gap-4 md:gap-5">
            {pains.map((t, i) => (
              <div key={t} className="reveal group glass rounded-2xl p-5 md:p-6 border border-white/8 flex gap-4 items-start hover:border-[#F239FF]/30 hover:-translate-y-0.5 transition-all duration-500" style={{ transitionDelay: `${i * 60}ms` }}>
                <div className="shrink-0 mt-0.5 w-9 h-9 rounded-xl flex items-center justify-center font-mono font-semibold text-[13px] text-white/85 gradient-border bg-black/40">
                  0{i + 1}
                </div>
                <p className="text-white/82 text-[15.5px] leading-relaxed pt-1">{t}</p>
              </div>
            ))}
          </div>
...
const LearnCards = () => {
  const cards = [
    { Icon: Target, tag: "Oportunidad", title: "La oportunidad real que tienen las agencias de IA hoy",
      body: "Hay miles de empresas que necesitan implementar IA pero no saben cómo hacerlo. Están dispuestas a pagar bien a quien pueda resolverlo — y tú puedes ser esa persona." },
    { Icon: Map, tag: "Sistema", title: "El mapa paso a paso para llegar a 5K€/mes",
      body: "Los 3 pilares que separan una agencia que cobra 500€ por proyecto de una que factura 5.000€ al mes de forma recurrente: oferta, adquisición y retención." },
    { Icon: Rocket, tag: "Ejecución", title: "Cómo pasar de aprender IA a cobrar por ello en 90 días",
      ...
      <div className="max-w-[1180px] mx-auto">
        <div className="text-center max-w-2xl mx-auto reveal">
          ...
          <h2 className="mt-4 font-bold text-[clamp(1.8rem,4.5vw,3rem)] leading-[1.05] tracking-[-0.02em]">
            ¿Qué vas a descubrir en<br />esta <span className="text-brand-gradient">masterclass</span>?
          </h2>
        </div>
...
const ForYou = () => {
  const yes = [
    "Sabes de IA pero todavía no facturas de forma consistente con eso",
    "Quieres construir una agencia de IA seria, no un hobby",
    "Estás cansado de buscar clientes sin un sistema",
    "Quieres un plan claro en vez de seguir improvisando",
    "No sabes programar pero quieres aprovechar la oportunidad de la IA",
    "Estás dispuesto a ejecutar si alguien te muestra el camino",
  ];
  const no = [
    "Buscas hacerte rico de la noche a la mañana sin trabajar",
          ...
          <div className="relative grid lg:grid-cols-[1fr_1.05fr] gap-10 lg:gap-14 items-center">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full brand-gradient text-[11px] tracking-[0.2em] uppercase font-bold text-white">
                <Gift className="w-3.5 h-3.5" /> Bonus al registrarte
              </div>
              <h2 className="mt-5 font-black text-[clamp(1.8rem,4.6vw,3rem)] leading-[1.05] tracking-[-0.02em]">
              ...
              <p className="mt-6 text-[#F239FF]/85 text-[14px] italic">
                No necesitas conocimientos técnicos para usarlo.
              </p>
              <button type="button" onClick={onOpenModal} className="mt-7 cta-primary rounded-full px-7 py-4 font-semibold text-white text-sm sm:text-base tracking-wide inline-flex items-center gap-2">
                QUIERO MI LUGAR + EL KIT <ArrowRight className="w-4 h-4" />
              </button>
            </div>

            {/* iOS-style folder visual */}
            <div className="relative" style={{ animation: "float 9s ease-in-out infinite" }}>
              <div className="absolute -inset-6 bg-gradient-to-br from-[#8943E3]/35 via-transparent to-[#F239FF]/30 blur-3xl rounded-[3rem]" />
              <div className="relative rounded-[2rem] p-5 md:p-6 overflow-hidden"
                   style={{
                     background: "linear-gradient(155deg, rgba(30,18,48,0.88) 0%, rgba(14,10,20,0.92) 100%)",
                     backdropFilter: "blur(24px) saturate(1.3)",
                     WebkitBackdropFilter: "blur(24px) saturate(1.3)",
                     border: "1px solid rgba(255,255,255,0.08)",
                     boxShadow: "0 30px 80px -20px rgba(137,67,227,0.45), inset 0 1px 0 rgba(255,255,255,0.08)",
                   }}>
                {/* Window header */}
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 sm:gap-4">
                  {items.map((item, i) => (
                    <div key={item.label}
                         className="relative group flex flex-col items-center gap-2.5 p-4 rounded-2xl bg-white/[0.025] border border-white/5 hover:bg-white/[0.05] hover:border-[#F239FF]/25 transition-all duration-500"
                         style={{ animation: `fadeUp .8s ${0.2 + i * 0.12}s cubic-bezier(.2,.7,.2,1) both` }}>
                      <div className="relative">
                        <div className="absolute inset-0 blur-xl opacity-60 brand-gradient rounded-[18px]" />
                        <div className="relative w-[64px] h-[64px] rounded-[18px] brand-gradient flex items-center justify-center"
                             style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.3), 0 10px 24px -6px rgba(242,57,255,0.55)" }}>
                          <item.Icon className="w-7 h-7 text-white" strokeWidth={1.8} />
                        </div>
                      </div>
                      <div className="text-[11.5px] font-semibold text-white/90 tracking-wide">{item.label}</div>
                    </div>
                  ))}
                </div>
      ...
      <div className="reveal relative gradient-border-strong rounded-[2rem] p-6 md:p-14 overflow-hidden text-center closing-pulse">
        ...
        <div className="relative">
          ...
          <button type="button" onClick={onOpenModal}
                  className="mt-8 cta-primary rounded-full px-8 py-4 font-semibold text-white text-sm sm:text-base tracking-wide inline-flex items-center gap-2">
            RESERVAR MI LUGAR <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
...
const Modal = ({ open, onClose, onSubmit, submitting }) => {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md"
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <h3 className="font-bold text-2xl md:text-[1.75rem] leading-tight tracking-[-0.01em]">
          Reserva tu plaza
        </h3>
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-5 space-y-4">
          <div>
            <div className={`flex items-center rounded-xl border bg-black/30 ${errors.name ? "border-rose-400/60" : "border-white/10"}`}>
          ...
          <div>
            <div className={`flex items-center rounded-xl border bg-black/30 ${errors.email ? "border-rose-400/60" : "border-white/10"}`}>
          ...
          <div className="flex items-center justify-center gap-2 text-xs text-white/45 pt-1">
            <Lock className="w-3.5 h-3.5" /> Tus datos están seguros.
          </div>
        </form>
      </div>
    </div>
...
const CALENDAR_URL = "https://calendar.google.com/calendar/render?action=TEMPLATE&text=Masterclass+Nexum+AI&dates=20260507T170000Z/20260507T183000Z&details=Masterclass+en+vivo+con+Agust%C3%ADn+Badt+y+Juan+Pablo+Rosso+de+Nexum+AI&location=Online";

const PostRegistration = () => {
  useEffect(() => {
    const redirect = setTimeout(() => {
      window.location.href = "https://chat.whatsapp.com/JgtmSpZqoSq3lwzDvfyJqc?mode=gi_t";
    }, 4000);
  ...
  <div style={{ animation: "fadeUp .9s cubic-bezier(.2,.7,.2,1) both" }}>
    ...
    <section className="relative pt-14 md:pt-24 pb-6 md:pb-10 px-4 sm:px-6">
      <div className="max-w-[760px] mx-auto text-center">
        ...
        <h1 className="mt-6 font-black text-[clamp(2.2rem,6vw,4.2rem)] leading-[1.02] tracking-[-0.02em]">
          ¡Ya estás dentro! <span style={{ animation: "float 8s ease-in-out infinite", display: "inline-block" }}>🎉</span>
        </h1>
    ...
    <section className="relative px-4 sm:px-6 pb-10 md:pb-14">
      <div className="max-w-[1080px] mx-auto">
        <div className="relative gradient-border-strong rounded-[2rem] overflow-hidden p-6 md:p-12 text-center whatsapp-pulse">
          <div className="relative max-w-[640px] mx-auto">
            ...
            <h2 className="mt-4 font-bold text-[clamp(1.7rem,4vw,2.6rem)] leading-[1.1] tracking-[-0.02em]">
              Únete al <span className="text-brand-gradient">grupo de WhatsApp</span>
            </h2>
            <p className="mt-4 text-white/70 text-[15.5px] leading-relaxed max-w-md mx-auto">
              Entrá al grupo para no perderte ningún detalle del evento en vivo.
            </p>
            <ul className="mt-7 space-y-3.5 text-left max-w-md mx-auto">
              {[
            ...
            <a href="https://chat.whatsapp.com/JgtmSpZqoSq3lwzDvfyJqc?mode=gi_t" target="_blank" rel="noopener noreferrer"
               className="cta-whatsapp mt-8 rounded-full px-7 py-3.5 font-semibold text-white text-sm sm:text-base tracking-wide inline-flex items-center justify-center gap-2.5">
...
export default function App() {
  ...
  const handleSubmit = useCallback((data) => {
    // Fire-and-forget a n8n. credentials:'omit' evita el preflight con
    // Allow-Credentials que rompía con sendBeacon. keepalive:true permite que
    // el POST sobreviva a la navegación / cierre de pestaña.
    const webhookUrl = "https://n8n.nexumai.online/webhook/57459119-11c9-4608-bc70-85b9193a9b88";
    const payload = { email: data?.email, nombre: data?.name };
    fetch(webhookUrl, {
      method: "POST",
      mode: "cors",
      credentials: "omit",
    ...
    // Marker en DevTools para confirmar que el handler corrió (Network → POST al webhook)
    // eslint-disable-next-line no-console
    console.log("[webinar] webhook n8n →", payload);
  ...
  return (
    <div className="relative min-h-screen bg-[#0A0A0A] text-white" style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, system-ui, sans-serif", letterSpacing: '-0.005em' }}>
      <style>{`
        ...
        .card-travel::before {
          ...
          background: conic-gradient(from var(--travel-angle, 0deg),
            ...
            rgba(242,57,255,0) 0.78turn,
            rgba(242,57,255,0.65) 0.86turn,
            rgba(255,210,255,0.95) 0.9turn,
            rgba(242,57,255,0.65) 0.94turn,
            transparent 1turn);
          -webkit-mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);
          -webkit-mask-composite: xor;
                  mask-composite: exclude;
          animation: travelAngle 7s linear infinite;
        ...
        .whatsapp-pulse::after {
          ...
          inset: -12%;
          background:
            radial-gradient(ellipse at 30% 50%, rgba(37,211,102,0.28) 0%, transparent 55%),
            radial-gradient(ellipse at 70% 50%, rgba(137,67,227,0.28) 0%, transparent 55%);
          filter: blur(22px);
          animation: whatsappPulse 10s ease-in-out infinite;
          pointer-events: none;
          z-index: 0;
        }
        .whatsapp-pulse > * { position: relative; z-index: 1; }
        @media (prefers-reduced-motion: reduce) {
          .whatsapp-pulse::after { animation: none; opacity: 0.5; transform: none; }
        }

        /* WhatsApp CTA — green ↔ violet gradient */
        .cta-whatsapp {
          background: linear-gradient(95deg, #25D366 0%, #1FAE5F 30%, #8943E3 95%);
          background-size: 200% 100%;
          box-shadow:
            0 12px 30px -10px rgba(37,211,102,0.5),
            0 12px 30px -10px rgba(137,67,227,0.4),
            0 0 0 1px rgba(255,255,255,0.1) inset;
          transition: transform .35s cubic-bezier(.2,.8,.2,1), box-shadow .35s, background-position .6s;
        }
        .cta-whatsapp:hover {
          transform: translateY(-2px) scale(1.02);
          background-position: 100% 50%;
          box-shadow:
            0 20px 50px -12px rgba(37,211,102,0.6),
            0 20px 50px -12px rgba(137,67,227,0.55),
            0 0 0 1px rgba(255,255,255,0.15) inset;
        }
      `}</style>
      {view === "registration" ? (
        <>
          <Nav countdown={countdown} onOpenModal={openModal} />
          <Hero countdown={countdown} onOpenModal={openModal} />
          <PainBlock />
          <LearnCards />
          <ForYou />
          <About />
          <Bonus onOpenModal={openModal} />
      ...
      <Footer />
      <Modal open={modalOpen} onClose={closeModal} onSubmit={handleSubmit} submitting={submitting} />
